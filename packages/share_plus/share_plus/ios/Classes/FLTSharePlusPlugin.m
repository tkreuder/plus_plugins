    ...
    [shareChannel
     setMethodCallHandler:^(FlutterMethodCall *call, FlutterResult result) {
        ...
        if ([@"share" isEqualToString:call.method] ||
            ...
            BOOL isCoordinateSpaceOfSourceView = CGRectContainsRect(topViewController.view.frame,originRect);
            if (!isCoordinateSpaceOfSourceView) {
                result([FlutterError errorWithCode:@"error"
                                           message: [NSString stringWithFormat: @"sharePositionOrigin: %@ must be within coordinate space of source view: %@", NSStringFromCGRect(originRect), NSStringFromCGRect(topViewController.view.bounds)]
                                           details:nil]);
                return;
            }
